...
public interface RoleMapper extends HeMapper<Role> {
    ...
    @Select("SELECT * FROM z10_user WHERE user_id IN (SELECT user_id FROM z11_user_role WHERE role_id = #{id}")
    List<User> listUser(String id);

    @Select("SELECT * FROM z10_menu WHERE menu_id IN (SELECT menu_id FROM z11_role_menu WHERE role_id = #{id}")
